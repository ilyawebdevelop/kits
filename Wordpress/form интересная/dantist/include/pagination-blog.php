...
<div class="blog__footer">
    <a href="<?= ( ! is_single() && ( $nextpage <= $max_num_pages ) ) ? next_posts( $max_page, false ): "javascript:void(0)";?>" class="blog__footer-link ajax-link"><?=$args["btn-text"]?></a>
</div>
<script>
    new Showmore({
        wrapperItems: "<?=$args["items-id"]?>", 
        wrapperPagination: "<?=$args["pagination-id"]?>",
        eventStop: "<?=$args["type-name"]?>"
    }).init()
</script>
<?endif;?>
<?/*
